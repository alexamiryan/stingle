...
class RewriteAliasMap extends DbAccessor {
	...
	public function getAliasMap(Host $host=null, $cacheMinutes = null){
		...
		$query = "SELECT a.*, ah.host_id FROM `".self::alias_tblname."` a
				  	LEFT JOIN `".self::alias_host_tblname."` ah ON  a.id = ah.alias_id
	...
	public function updateAlias($id, $alias, $map, Host $host=null){
		...
		$this->query->exec("UPDATE `".self::alias_tblname."` SET alias='$alias', map = '$map' WHERE id='$id'");
		if($host !== null){
			$this->query->exec("UPDATE `".self::alias_host_tblname."` SET host_id='{$host->id}'  WHERE alias_id='$id'");
		}
	}
	...
	public function updateMap($id, $map){
		...
		$this->query->exec("UPDATE `".self::alias_tblname."` SET map='$map' WHERE id='$id'");
	}
	...
	public function addAlias($alias, $map, Host $host){
		...
		$this->query->exec("INSERT INTO `".self::alias_tblname."` (`alias`, `map`) VALUES ('$alias', '$map')");
		$inser_id = $this->query->getLastInsertId();
		$this->query->exec("INSERT INTO `".self::alias_host_tblname."` (`host_id`, `alias_id`) VALUES ('$host->id', '$inser_id')");
	}
	...
	public function deleteAlais($id){
		...
		$this->query->exec("DELETE FROM `".self::alias_tblname."` WHERE `id` = '$id'");
	}
	/**
	...
	public function getKnownAliasHosts($cacheMinutes = null){
		$hosts = array();
		$query = "SELECT DISTINCT host_id FROM `".self::alias_host_tblname."`";
		$this->query->exec($query, $cacheMinutes);
	...
	public function getAliasHost($id, $cacheMinutes = null){
		$this->query->exec("SELECT `host_id` FROM `".self::alias_host_tblname."` WHERE `alias_id`={$id}", $cacheMinutes);
