...
class FormSecurity extends DbAccessor {
	...
	private function isBlockedClient() {
		...
		if($this->userid > 0) {
			$this->query->exec("SELECT (UNIX_TIMESTAMP(unblocktime) - UNIX_TIMESTAMP(CURRENT_TIMESTAMP())) as blocktime FROM `".self::TBL_SECURITY_USERS."` 
		...
		$this->query->exec("SELECT (UNIX_TIMESTAMP(unblocktime) - UNIX_TIMESTAMP(CURRENT_TIMESTAMP())) as blocktime  FROM `".self::TBL_SECURITY_HOSTS."` 
		...
		$this->query->exec("SELECT (UNIX_TIMESTAMP(unblocktime) - UNIX_TIMESTAMP(CURRENT_TIMESTAMP())) as blocktime  FROM `".self::TBL_SECURITY_SESSIONS."` 
	...
	private function checkUserStatus() {
		...
		if ( $userid > 0 ) {
			...
			$this->query->exec("SELECT (UNIX_TIMESTAMP(request_time)-UNIX_TIMESTAMP(counter_last_reset_time)) as timediff,counter,blockstatus FROM `".self::TBL_SECURITY_USERS."` 
								WHERE user_id = '".$userid."' AND label='".$this->label."'");
			
			if ( $this->query->countRecords() == 0 ) {
				$this->query->exec("INSERT INTO `".self::TBL_SECURITY_USERS."` (user_id,counter_last_reset_time,label)
				...
				if(((int)$db_blockstatus) == 0) {					
					if( $db_timediff >= $this->allowedUserTime) {
						if( ( $blockTimeSec = $this->getBlockTimeFromConf ( $this->allowedUserAttempts, $db_counter ) ) > 0 ) {
							...
							$this->query->exec("UPDATE `".self::TBL_SECURITY_USERS."` SET counter=0,counter_last_reset_time=CURRENT_TIMESTAMP()
												WHERE user_id=".$userid." AND label='".$this->label."' AND blockstatus = 0");
						}				 
					}
					$this->query->exec("UPDATE `".self::TBL_SECURITY_USERS."` SET counter=counter + 1 
										WHERE user_id=".$userid." AND label='".$this->label."' AND blockstatus = 0");
				}										
			}
			return true;	
	...
	private function checkHostStatus() {
		...
		$this->query->exec("SELECT (UNIX_TIMESTAMP(request_time)-UNIX_TIMESTAMP(counter_last_reset_time)) as timediff,counter,blockstatus  
							FROM `".self::TBL_SECURITY_HOSTS."` WHERE remote_ip = '".$this->clientHostIp."' 
							AND forwarded_ip='".$this->clientForwardedIp."'
							AND label='".$this->label."'");
		
		if ( $this->query->countRecords() == 0 ) {
			$this->query->exec("INSERT INTO `".self::TBL_SECURITY_HOSTS."` (remote_ip,forwarded_ip,counter_last_reset_time,label)
			...
			if(((int)$db_blockstatus) == 0) {	
				if( $db_timediff >= $this->allowedHostTime) {
					if( ( $blockTimeSec = $this->getBlockTimeFromConf ( $this->allowedHostAttempts, $db_counter ) ) > 0 ) {
						...
						$this->query->exec("UPDATE `".self::TBL_SECURITY_HOSTS."` 
				...
				$this->query->exec("UPDATE `".self::TBL_SECURITY_HOSTS."` SET counter=counter + 1 
									WHERE remote_ip = '".$this->clientHostIp."' 
									AND forwarded_ip = '".$this->clientForwardedIp."'
									AND label='".$this->label."' AND blockstatus = 0");			
			}					
	...
	private function checkSessionStatus() {
		...
		$this->query->exec("SELECT (UNIX_TIMESTAMP(request_time)-UNIX_TIMESTAMP(counter_last_reset_time)) as timediff,counter,blockstatus 
							FROM `".self::TBL_SECURITY_SESSIONS."` WHERE session_id = '".$this->session_id."' AND label='".$this->label."'");
	
		if ( $this->query->countRecords() == 0 ) {
			$this->query->exec("INSERT INTO `".self::TBL_SECURITY_SESSIONS."` (session_id,remote_ip,counter_last_reset_time,label)
			...
			if(((int)$db_blockstatus) == 0) {
				if( $db_timediff >= $this->allowedSessionTime) {
					if( ( $blockTimeSec = $this->getBlockTimeFromConf ( $this->allowedSessionAttempts, $db_counter ) ) > 0 ) {
						...
						$this->query->exec("UPDATE `".self::TBL_SECURITY_SESSIONS."` 
											SET counter=0,counter_last_reset_time=CURRENT_TIMESTAMP() 
											WHERE session_id = '".$this->session_id."'
											AND label='".$this->label."' AND blockstatus = 0");
					}				 
				}
				$this->query->exec("UPDATE `".self::TBL_SECURITY_SESSIONS."` SET counter=counter + 1 
									WHERE session_id = '".$this->session_id."'
									AND label='".$this->label."' AND blockstatus = 0");
			}							
		}
	...
	private function unblockElapsedUser() {
		...
		if ($this->userid > 0) {
			...
			$this->query->exec("UPDATE `".self::TBL_SECURITY_USERS."` 
