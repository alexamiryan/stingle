...
class IpFilter extends DbAccessor {
	...
	private function checkIpInBlockedIpList () {
		...
		$this->query->exec("SELECT id FROM `".Tbl::get('TBL_SECURITY_BLOCKEDIPS')."`
	...
	private function checkIpInBlockedCountries () {
		...
		$this->query->exec("SELECT id FROM `".Tbl::get('TBL_SECURITY_BLOCKEDCOUNTRIES')."` 
	...
	private function checkIpInBlockedIpRanges() { 
		...
		$this->query->exec("SELECT ip_range FROM `".Tbl::get('TBL_SECURITY_BLOCKEDIPRANGES')."`");
